...
- **🔴 NO BRAIN ARTIFACTS 🔴**: NEVER use the `brain` directory for artifacts. All artifacts MUST be in `./multi-agent/core/artifacts/`.
...
5. **IGNORE EXTERNAL**: NEVER read or investigate `./communication/`. It is out of scope.

## ⛔ CRITICAL: DATA SOURCES & WORKSPACE ⛔
- **AGENT WORKSPACE RESTRICTION**: 
  - **READ-ONLY**: You have read-only access to `./multi-agent/core/*.md`, `./multi-agent/core/*.py`, and `./multi-agent/workflows/*`. **NEVER** modify these files.
  - **READ-WRITE**: You have full access to `./multi-agent/core/artifacts/`, `./multi-agent/core/specs/`, `./multi-agent/core/history/`, and `./multi-agent/status.json`.
...
- **🔴 ARTIFACT PORTABILITY 🔴**: NEVER create artifacts in brain directory (`~/.gemini/antigravity/brain/...`). Only `./multi-agent/core/artifacts/` and `./multi-agent/core/specs/` are allowed. Files in brain directory are not portable and will disappear.
...
## ROLE BOUNDARIES:
- **🔴 NO BRAIN LEAK 🔴**: NEVER write files to an IDE-private or external directory. Always use local project paths like `./multi-agent/core/artifacts/`. **Antigravity only**: NEVER use `IsArtifact: true` in `write_to_file` calls — this hides files in the IDE's private brain directory, making them invisible to other agents.
...
- **Real-Time Check**: Always use `cat` in terminal to read `./multi-agent/status.json` AFTER the polling script finishes.
