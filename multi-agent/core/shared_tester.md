...
🔴 **SILENCE** 🔴
- `cat ./multi-agent/status.json` to confirm Tester status is exactly `ready`.
...
- If `qa_attempts` > max retries:
  ...
  2. Extract `TASK_ID` from the `Task ID:` field in `task.md`, then run `python3 multi-agent/core/archive_task.py <TASK_ID> failed`
  3. Run `python3 multi-agent/core/pop_task.py`
  4. **🔴 CLEANUP ARTIFACTS 🔴**: Delete `implementation_plan.md`, `task.md`, `qa_feedback.md` from `./multi-agent/core/artifacts/`
...
🔴 **SILENCE** 🔴
- Read `./multi-agent/core/artifacts/task.md` — understand what was requested (Stack, Task ID, requirements).
- Read `./multi-agent/core/artifacts/implementation_plan.md` — understand what Developer was supposed to implement.
- Check `./multi-agent/core/artifacts/qa_feedback.md` if it exists — this is feedback from a previous failed QA round on the same task.
...
1. **Update `status.json`** first so `archive_task.py` captures final Tester timestamps:
   ...
   - **Antigravity**: use `write_to_file`. **Cursor (Claude Code)**: use the `Write` tool. **Codex**: use regular local filesystem edits.
2. **Archive Task**: Run `python3 multi-agent/core/archive_task.py <TASK_ID> completed`
   - `TASK_ID` is from the `Task ID:` field in `task.md` (the real numeric ID, NOT a position number).
3. **Pop Task**: Run `python3 multi-agent/core/pop_task.py`
4. **🔴 CLEANUP ARTIFACTS 🔴**:
