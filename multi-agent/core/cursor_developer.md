...
Monitor(
  ...
  command="while true; do
    status=$(python3 -c \"import json; d=json.load(open('multi-agent/status.json')); print(next(r['status'] for r in d['pipeline'] if r['role']=='Developer'))\" 2>/dev/null)
    [ \"$status\" = 'ready' ] && echo 'DEVELOPER_READY' && exit 0
    sleep 3
  done"
)
```
