...
sys.path.append(os.path.join(NOX_ROOT, "multi-agent/core"))
...
PIPER_MODEL = os.path.join(NOX_ROOT, "piper_models/en_GB-alan-medium.onnx")
PIPER_EXE = os.path.join(NOX_ROOT, "venv/bin/piper")
TEMP_WAV = "/tmp/nox_raw.wav"
EFFECT_WAV = "/tmp/nox_fx.wav"

MEMORY_FILE = os.path.join(NOX_ROOT, "database", "brain_memory.json")
HISTORY_FILE = os.path.join(NOX_ROOT, "database", "chat_history.json")
...
async def kill_agent(request: dict):
    ...
    try:
        ...
        if role:
            hb = os.path.join(NOX_ROOT, f"multi-agent/.runtime/heartbeat_{role.lower()}.json")
            try:
                os.remove(hb)
