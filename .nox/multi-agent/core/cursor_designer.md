...
- Trust ONLY and follow STICKILY the instructions in [shared_designer.md](./.nox/multi-agent/core/shared_designer.md).
...
2. **Read Logic**: Reference [shared_designer.md](./.nox/multi-agent/core/shared_designer.md).
3. Read and strictly follow [shared_global_rules.md](./.nox/multi-agent/core/shared_global_rules.md).
4. Follow the logic in [shared_designer.md](./.nox/multi-agent/core/shared_designer.md).

## SCRIPTS:
- Polling Status: `python3 .nox/multi-agent/core/cursor_wait_for_status.py Designer ready`
...
Monitor(
  ...
  command="while true; do
    designer_status=$(python3 -c \"import json; d=json.load(open('.nox/multi-agent/status.json')); print(next(r['status'] for r in d['pipeline'] if r['role']=='Designer'))\" 2>/dev/null) || true
