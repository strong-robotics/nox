...
def log(role: str, msg: str):
    try:
        log_file = RUNTIME_DIR / f"vscode_{role.lower()}.log"
...
def wait_for_status(role, target_status):
    print(f"--- VSCODE AUTONOMOUS MODE: {role.upper()} ---")
    ...
    while True:
        ...
        # Check 3: VSCode/Copilot no longer in parent chain (every ~10s)
        check_counter += 1
        if check_counter % 4 == 0:
            if not has_copilot_parent():
                log(role, "EXIT: VSCode/Copilot gone from parent chain")
                sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 vscode_wait_for_status.py [Role] [Status] [optional: status_file_path]")
        sys.exit(1)
